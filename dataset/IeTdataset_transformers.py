...
def generate_event_image(x, y, p):  
    ...
    for x_, y_, p_ in zip(x, y, p):
        if p_ == 0:
            event_image[y_, x_] = np.array([0, 0, 255])  # blue
        else:
            event_image[y_, x_] = np.array([255, 0, 0])  # red
...
class EventChatDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in self.list_data_dict[i] and not getattr(self.data_args, 'event_folder', None) and not self.data_args.spatial_temporal_encoder:
