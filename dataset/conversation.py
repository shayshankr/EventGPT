...
class Conversation:
    ...
    def get_prompt(self):
        ...
        if len(messages) > 0 and type(messages[0][1]) is tuple:
            ...
            init_msg = init_msg[0].replace("<event>", "").strip()
            if 'mmtag' in self.version:
                messages[0] = (init_role, init_msg)
                messages.insert(0, (self.roles[0], "<Event><event></Event>"))
                messages.insert(1, (self.roles[1], "Received."))
            else:
                messages[0] = (init_role, "<event>\n" + init_msg)
...
def prepare_event_prompt(query, conv_mode):
